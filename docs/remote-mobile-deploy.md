...
电脑端也提供了快捷方式：在 Agent 的 `.env` 配好 Supabase 地址和 service role key 后，Windows 双击 `remote-system/agent/issue_invitation_codes.bat`，macOS 双击 `remote-system/agent/issue_invitation_codes.command`，窗口会直接打印新邀请码；默认生成 10 个，可用 `--count 20` 调整数量。管理密钥只留在电脑端。
