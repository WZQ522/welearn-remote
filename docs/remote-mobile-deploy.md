...
电脑端也提供了快捷方式：macOS 第一次双击 `remote-system/agent/issue_invitation_codes.command` 时会弹窗填写 Supabase 地址和 service role key，并自动保存到本地；以后直接双击即可。Windows 双击 `remote-system/agent/issue_invitation_codes.bat` 前需先填写 Agent 的 `.env`。默认生成 10 个，可用 `--count 20` 调整数量。管理密钥只留在电脑端。
